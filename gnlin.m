...
!copy x13.m nfx.m;
X=xlsread('Chwirut1','a2:b55');
%x1=xlsread('data5','a1:a3424');
%x2=xlsread('data5','b1:b3424');
%y=xlsread('data5','c1:c3424');
%X=[x1 x2 x3 y];
[n,dim]=size(X);
y=X(:,dim);dim=dim-1;
k=3;
N=ceil(280+23*k^2);
b0=rand(1,k);
%b0=[-591.0850468 593.8135196 -0.000487544896];
b0=[0.1665764858 0.005165326293 0.01215001412];
bi=.017*b0;bm=b0;bo=b0;b=b0;
SSy=var(y)*(n-1);
if dim==1
    x=X(:,1);x1a=min(x);x1b=max(x);r1=range(x);
    x11=linspace(x1a-r1/1000,x1b+r1/50,350);
elseif dim==2
    x1=X(:,1);x2=X(:,2);
...
while (qmn(v0-3)-qmn(v0))/qmn(v0)>=1e-12 && rep<=3
    ...
    if (qm(v1-5)-qm(v1))/qm(v1)<k*k*1e-10
